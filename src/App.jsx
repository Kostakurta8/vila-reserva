...
import { generateId, formatLong, formatShort, nightsBetween } from './utils/dateHelpers';
import { checkBookingConflict, validateBooking } from './utils/validation';
import { useReservations } from './hooks/useReservations';
import { useRooms } from './hooks/useRooms';
import { apiLogin, apiVerifyToken, apiLogout } from './hooks/useApi';
...
function AppContent({ onLogout }) {
  ...
  /* ── Rooms from API ────────────────────────────────────────── */
  const { rooms, loading: roomsLoading, addRoom, updateRoom: updateRoomApi, deleteRoom: deleteRoomApi } = useRooms();
  ...
  }, [editRes, modal, reservations, t, showToast, addReservation, updateReservation, closeModal]);
  ...
  const filteredRes = useMemo(() => {
    ...
    if (filterStatus !== 'All') list = list.filter((r) => r.paymentStatus === filterStatus);
    ...
    return list;
  }, [reservations, filterStatus, search, sortBy, dateFrom, dateTo, timeFilter, rooms, today]);
  ...
  return (
    <>
      <div className="app">
        <Sidebar
          t={t} lang={lang} setLang={setLang} view={view}
          setView={(v) => { setView(v); closeSidebar(); }}
          rooms={rooms} open={sidebarOpen} onClose={closeSidebar}
          arrivals={arrivals.length}
          freeCount={freeCount}
          activeResCount={reservations.filter(r => r.checkOut >= today).length}
          onLogout={onLogout}
        />
        {sidebarOpen && <div className="sb-overlay" onClick={closeSidebar} onKeyDown={closeSidebar} role="presentation" />}

        <div className="main">
          <Topbar t={t} view={view} lang={lang} onNewBooking={() => openAdd()} onToggleSidebar={toggleSidebar}
            freeCount={freeCount} occupiedCount={occupiedIds.size} arrivalsCount={arrivals.length} />
