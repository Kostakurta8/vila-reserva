...
import {
  ...
  updateReservation, deleteReservation, cyclePaymentStatus,
} from './db.js';
...
app.post('/api/rooms', async (req, res) => {
  try {
    ...
    if (!id || !name?.trim() || !type || basePrice == null) {
      return res.status(400).json({ error: 'Missing required fields: id, name, type, basePrice' });
    }
...
app.post('/api/reservations', async (req, res) => {
  try {
    ...
    if (!id || !roomId || !guestName?.trim() || !checkIn || !checkOut) {
      return res.status(400).json({ error: 'Missing required fields: id, roomId, guestName, checkIn, checkOut' });
    }
    if (checkIn >= checkOut) {
